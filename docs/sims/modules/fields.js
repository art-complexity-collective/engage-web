...
export class ScalarField{
    ...
    add_gaussian_source(x,y, threshold, std, peak_val){
        ...
        for(let xt = xmin; xt < xmax; xt += 1){
            for(let yt = ymin; yt < ymax; yt += 1 ){
                ...
                if(dist<threshold){
                    let val = peak_val*gaussian(dist,std);
                    this._values[this._props.width*yt+xt] += val;
                }
            }
        }
        this.normalize_and_update(x,y,threshold, peak_val);
    }

    normalize_and_update(x, y, threshold, peak_val){
        ...
        for(let xt = xmin; xt < xmax; xt += 1){
            for(let yt = ymin; yt < ymax; yt += 1 ){
                ...
                if(dist<threshold){
                    ...
                    let [r,g,b,a] = gradient(0,peak_val,"#ffffff","#ff0000",value);
                    // Red.
    ...
    get_value(x,y){
        return this._values[x+this._props.width*y];
    }
